class Defaults {
    public static readonly clientSize = 30;	//pixels (diameter)
    public static readonly clientsSpeed = 2;	//messages per second
    public static readonly defaultColor = 'black';
    public static readonly frameRate = 60;		//frames per second
    public static readonly gameLength = 5; //minutes
    public static readonly maxClientWaitTime = 9;	//seconds
    public static readonly messageVelocity = 200;	//pixels per second
    public static readonly serversCapacity = 80; //messages
    public static readonly serverSize = 40;	//pixels (side)
    public static readonly serversSpeed = 3.5;	//messages per second
    public static readonly gameModes = { MENU: 0, GAME: 1, GAMEOVER: 2, CREDITS: 3, PAUSE: 4, UPGRADE: 5, TUTORIAL: 6 };
}